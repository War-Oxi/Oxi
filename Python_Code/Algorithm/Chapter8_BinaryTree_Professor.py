...
class BSTNode:
    ...
    def insertNode(self, keyvalue):
        if self.keyvalue.key == keyvalue.key:
            ...
            if self.left:  # self.left != None
                self.left.insertNode(keyvalue)  # 재귀 사용
                return
            self.left = BSTNode(keyvalue)
        else:  # keyvalue.key > self.keyvalue.key
            if self.right:  # self.right != None
    ...
    def printNode(self, depth, which):
        if self.left is not None:
            self.left.printNode(depth + 1, "left of " + self.keyvalue.key)
        print("depth %d, %s: " % (depth, which), end='')
        self.keyvalue.printKeyValue()
        if self.right is not None:
            self.right.printNode(depth + 1, "right of " + self.keyvalue.key)
        ...
        while current is not None:
            if key < current.keyvalue.key:
                ...
                if current.left is not None and current.right is not None:
                    ...
                    current.keyvalue = min_larger_node.keyvalue  # current = 삭제할 값의 위치
                    #삭제할 값이 루트 노드일 경우
                    if min_larger_node == current.right:
                        parent.right = min_larger_node.right
                    else:  # min_larger_node != current.right  # min_larger_node == current.left??
